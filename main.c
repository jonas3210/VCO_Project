...
int main(void) {
	...
	while (1) {
		...
		Get_ADC_Values();
		Get_Mean_Values();
		if (INPUT_BUTTON_OUT1 && !Menu_Flag && !INPUT_BUTTON_HOLD) {//Calculation of VCO Parameters with Input Values
			...
			if (!VCO1_Lock.Freq_Lock & VCO1_Lock.Freq_Fine_Lock) {
				VCO1.Phase_Shift =
						DOUBLE_TO_FIXED(CV_FACTOR_1 * pow(2,(INPUT_FREQUENCY_COARSE_MEAN_VALUE) * CV_FACTOR_2)*2); //Refresh Coarse Phase scaling for 1V/Octave CV
				VCO1.Octave_Index =
						DOUBLE_TO_FIXED((INPUT_FREQUENCY_COARSE_MEAN_VALUE)/409.5);
			}
			//TODO: seperate Fine Freq Calculation from Coarse Calculation
			if (!VCO1_Lock.Freq_Fine_Lock) {
				VCO1.Phase_Shift =
						DOUBLE_TO_FIXED(CV_FACTOR_1 * pow(2,(INPUT_FREQUENCY_COARSE_MEAN_VALUE+((INPUT_FREQUENCY_FINE_MEAN_VALUE>>3)-256)) * CV_FACTOR_2)*2); //REfresh Coarse and fine Phase scaling for 1V/Octave CV
				VCO1.Octave_Index =
						DOUBLE_TO_FIXED((INPUT_FREQUENCY_COARSE_MEAN_VALUE+((INPUT_FREQUENCY_FINE_MEAN_VALUE>>3)-256))/409.5);
			}
		...
		if (INPUT_BUTTON_OUT2 && !Menu_Flag && !INPUT_BUTTON_HOLD) {
			...
			if (!VCO2_Lock.Freq_Lock & VCO2_Lock.Freq_Fine_Lock) {
				VCO2.Phase_Shift =
						DOUBLE_TO_FIXED(CV_FACTOR_1 * pow(2,(INPUT_FREQUENCY_COARSE_MEAN_VALUE) * CV_FACTOR_2)*2); //Phase scaling for 1V/Octave CV
				VCO2.Octave_Index =
						DOUBLE_TO_FIXED((INPUT_FREQUENCY_COARSE_MEAN_VALUE)/409.5);
			}
			if (!VCO2_Lock.Freq_Fine_Lock) {
				VCO2.Phase_Shift =
						DOUBLE_TO_FIXED(CV_FACTOR_1 * pow(2,(INPUT_FREQUENCY_COARSE_MEAN_VALUE+((INPUT_FREQUENCY_FINE_MEAN_VALUE>>3)-256)) * CV_FACTOR_2)*2); //Refresh Coarse Phase scaling for 1V/Octave CV
				VCO2.Octave_Index =
						DOUBLE_TO_FIXED((INPUT_FREQUENCY_COARSE_MEAN_VALUE+((INPUT_FREQUENCY_FINE_MEAN_VALUE>>3)-256))/409.5);
			}
		...
		if (Transfer_Complete_Flag) {
			for (int i = 0; i < I2S_BUFFERSIZE / 2; i++) {
				...
				VCO1_Output_Buffer[i] = Mix_Modulation(VCO1_Waveshaped_Sample,
						VCO2_Waveshaped_Sample, VCO1.Mix_Value); //Calculates Mix Modulation
				VCO2_Output_Buffer[i] = Mix_Modulation(VCO2_Waveshaped_Sample,
						VCO1_Waveshaped_Sample, VCO2.Mix_Value); //
...
int32_t Subtable_Interpolation(int32_t Accumulator, int32_t Ocatve_Index,
	...
	if (Integer_Part < SUBTABLE_SIZE - 2)
		...
		High_Subtable_Sample = Low_Subtable_Sample;
